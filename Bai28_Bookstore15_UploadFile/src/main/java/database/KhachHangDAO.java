...
public class KhachHangDAO implements DAOInterface<KhachHang> {
	...
	public ArrayList<KhachHang> selectAll() {
		...
		try {
			...
			while (rs.next()) {
				...
				Date hieuLucMaXacThuc = rs.getDate("hieulucmaxacthuc");
				...
				KhachHang kh = new KhachHang(maKhacHang, tenDangNhap, matKhau, hoVaTen, gioiTinh, diaChi,
						...
						hieuLucMaXacThuc, trangThaiXacThuc, duongDanAnh);
				ketQua.add(kh);
			}
		...
		try {
			...
			while (rs.next()) {
				...
				ketQua = new KhachHang(maKhacHang, tenDangNhap, matKhau, hoVaTen, gioiTinh, diaChi, diaChiNhanHang,
						diaChiMuaHang, ngaySinh, soDienThoai, email, dangKyNhanBangTin, maXacThuc, hieuLucMaXacThuc,
	...
	public int insert(KhachHang t) {
		...
		try {
			...
			// Bước 2: tạo ra đối tượng statement
			String sql = "INSERT INTO khachhang (makhachhang, tendangnhap, matkhau, hoten, gioitinh, diachi, diachinhanhang, diachimuahang, ngaysinh, sodienthoai, email, dangkynhanbangtin) "
					+ " VALUES (?,?,?,?,?,?,?,?,?,?,?,?)";
	...
	public int updateVerifyInformation(KhachHang t) {
		...
		try {
			...
			// Bước 2: tạo ra đối tượng statement
			String sql = "UPDATE khachhang " + " SET " + " maxacthuc=?" + ", hieulucmaxacthuc=?"
					+ ", trangthaixacthuc=?" + " WHERE makhachhang=?";
			PreparedStatement st = con.prepareStatement(sql);
			st.setString(1, t.getMaXacThuc());
			st.setDate(2, t.getHieuLucMaXacThuc());
			...
			// Bước 4:
			System.out.println("Bạn đã thực thi xác thực tài khoản: " + sql);
			System.out.println("Có " + ketQua + " dòng bị thay đổi!");
	...
	public boolean changePasswordForKH(KhachHang t) {
		int ketQua = 0;
		try {
			// Bước 1: tạo kết nối đến CSDL
			Connection con = JDBCUtil.getConnection();
	...
	public KhachHang selectByUserAndPassWord(KhachHang t) {
		...
		try {
			...
			/*
			 * // Test SQL Injection Statement st = (Statement)con.createStatement(); String
			 * sql = "SELECT * FROM khachhang WHERE tendangnhap='"+t.getTenDangNhap()
			 * +"' and matkhau='" +t.getMatKhau()+"'"; // Bước 3: thực thi câu lệnh SQL
			 * System.out.println(sql); ResultSet rs = st.executeQuery(sql);
			 * 
			 * 
			 */
			// Bước 2: tạo ra đối tượng statement
	...
	/*
	 * public static void main(String[] args) { String maKhachHang =
	 * "1711714303499";
	 * 
	 * KhachHangDAO khachHangDAO = new KhachHangDAO();
	 * 
	 * KhachHang kh = new KhachHang(); kh.setMaKhachHang(maKhachHang); kh =
	 * khachHangDAO.selectById(kh); System.out.println(kh.toString());
	 * 
	 * String msg = ""; if (kh != null) { // Kiem tra ma xac thuc co giong nhau hay
	 * khong? // Kiem tra xem ma xac thuc con // hieu luc hay khong? // Thanh Cong
	 * kh.setTrangThaiXacThuc(true); khachHangDAO.updateVerifyInformation(kh); } }/*
	 * ArrayList<KhachHang> kh = khd.selectAll(); for(KhachHang khh : kh) {
	 * System.out.println(khh); } // KhachHang khachHang = new KhachHang(); //
	 * khachHang.setMaKhachHang("KH6"); // khachHang.setTenDangNhap("hihihaha"); //
	 * khachHang.setMatKhau("1234"); // khachHang.setHoVaTen("Nguyễn Văn Rỗi"); //
	 * khachHang.setDangKyNhanBangTin(false); // // khd.insert(khachHang); // kh =
	 * khd.selectAll(); // for(KhachHang khh : kh) // { // System.out.println(khh);
	 * // } }
	 * 
	 */
}