	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (khachHang != null) {
			try {
				...
				int maxMemSize = 5000 * 1024;// 5MB bộ nhớ
				...
				if (contentType.indexOf(contentType) >= 0) {
					...
					for (FileItem fileItem : files) {
						if (!fileItem.isFormField()) {
							...
							request.setAttribute("baoLoi", "Thay đổi ảnh đại diện thành công!");
							request.getSession().setAttribute("loixacthuc", true);
							url = "/KhachHang/thongbao.jsp";
						} else {
							String name = fileItem.getFieldName();
							String value = fileItem.getString();
							System.out.println(name + " : " + value);
							request.setAttribute("baoLoi", "Thay đổi ảnh đại diện thất bại");
							request.getSession().setAttribute("loixacthuc", true);
							url="/KhachHang/thongbao.jsp";
						}
					}
